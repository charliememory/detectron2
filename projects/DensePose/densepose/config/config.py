...
def add_densepose_head_config(cfg: CN):
    ...
    _C.MODEL.ROI_DENSEPOSE_HEAD.BODY_WEIGHTS = 5.0
    # Loss weights for surface parts. (24 Parts)
    _C.MODEL.ROI_DENSEPOSE_HEAD.PART_WEIGHTS = 1.0
    _C.MODEL.ROI_DENSEPOSE_HEAD.PART_FOCAL_LOSS = False
    _C.MODEL.ROI_DENSEPOSE_HEAD.PART_FOCAL_GAMMA = 1.0
    _C.MODEL.ROI_DENSEPOSE_HEAD.SMOOTH_WEIGHTS = 0.
    _C.MODEL.ROI_DENSEPOSE_HEAD.TV_WEIGHTS = 0.
    # Loss weights for UV regression.
    _C.MODEL.ROI_DENSEPOSE_HEAD.POINT_REGRESSION_WEIGHTS = 0.01
    _C.MODEL.ROI_DENSEPOSE_HEAD.MEAN_UV_LOSS = False
    # Coarse segmentation is trained using instance segmentation task data
    _C.MODEL.ROI_DENSEPOSE_HEAD.COARSE_SEGM_TRAINED_BY_MASKS = False

    _C.MODEL.ROI_DENSEPOSE_HEAD.AUX_REL_COORDS_WEIGHTS = 0.
    _C.MODEL.ROI_DENSEPOSE_HEAD.AUX_FG_SEGM_WEIGHTS = 0.
