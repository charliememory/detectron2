...
class DensePoseChartGlobalIUVSeparatedSMultiscaleCropResizeLoss(DensePoseChartGlobalIUVSeparatedSCropResizeLoss):
    """"""


    def produce_densepose_losses(
        self, proposals_with_gt: List[Instances], densepose_predictor_outputs_list: List[Any],
        gt_bitmasks: Any,
    ) -> LossDict:
        """
        ...
        losses_all = {"loss_densepose_U":[],
                      "loss_densepose_V":[],
                      "loss_densepose_I":[],
                      "loss_densepose_S":[]}
        for densepose_predictor_outputs in densepose_predictor_outputs_list:
            ...
            losses = {**losses_uv, **losses_segm}

            for k,v in losses.items():
                # pdb.set_trace()
                losses_all[k].append(v)
        for k in losses_all.keys():
            losses_all[k] = torch.stack(losses_all[k]).mean()
        return losses_all
        # else:
        #     return {**losses_uv}
