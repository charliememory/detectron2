...
class DensePoseDeepLabSparseHead(nn.Module):
    """
    ...
    def __init__(self, cfg: CfgNode, input_channels: int):
        super(DensePoseDeepLabSparseHead, self).__init__()
        # fmt: off
        ...
        self.ASPP = ASPP(input_channels, [6, 12, 56], n_channels, conv, deconv)  # 6, 12, 56
...
class ASPPPooling(nn.Sequential):
    def __init__(self, in_channels, out_channels, conv, deconv):
            # nn.AdaptiveAvgPool2d(1),
        super(ASPPPooling, self).__init__(
            spconv.SparseMaxPool2d(1),
            conv(in_channels, out_channels, 1, indice_key="subm0"),
            deconv(out_channels, out_channels, 1, indice_key="subm0"),
        )
...
class ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, out_channels, conv, deconv):
        super(ASPP, self).__init__()
        layers = []
        layers.append(conv(in_channels, out_channels, kernel_size=3, indice_key="subm0"))

        rate1, rate2, rate3 = tuple(atrous_rates)
        layers.append(conv(in_channels, out_channels, kernel_size=3, dilation=rate1, indice_key="subm0"))
        layers.append(conv(in_channels, out_channels, kernel_size=3, dilation=rate2, indice_key="subm0"))
        layers.append(conv(in_channels, out_channels, kernel_size=3, dilation=rate3, indice_key="subm0"))
        layers.append(ASPPPooling(in_channels, out_channels, conv, deconv))

        self.convs = nn.Sequential(*layers)

        self.project = conv(len(layers) * out_channels, out_channels, kernel_size=1, indice_key="subm0")
